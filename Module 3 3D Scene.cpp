...
namespace
{
    const char* const WINDOW_TITLE = "Module 3 3D Scene BHobbs"; // Macro for window title
    ...
    //Cylinder
    GLMesh cylinderMesh;
    Cylinder cylinder(0.5f, 0.4f, 1.5f, 36, 8, true, 2); // baseRadius, topRadius, height, sectors, stacks, flat shading, up-axis=Y(2);
...
int main(int argc, char* argv[])
{
    ...
    createPyramidMesh(pyramidMesh); // Calls the function to create the Vertex Buffer Object
    ...
    while (!glfwWindowShouldClose(gWindow))
    {
        ...
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); //use wireframe for QA GL_FILL for off GL_LINE for on
        URender();
...
void URender()
{
    ...
    // Draws the triangles
    glDrawElements(GL_TRIANGLES, pyramidMesh.nIndices, GL_UNSIGNED_SHORT, NULL); // Draws the triangle
...
// Implements the UCreateMesh function for the Cylinder
void createCylinderMesh(GLMesh& mesh)
{

    //Create the VAO for the Cylinder
    glGenVertexArrays(1, &mesh.vao); // we can also generate multiple VAOs or buffers at the same time
    glBindVertexArray(mesh.vao);

    // Create 2 buffers for the cylinder: first one for the vertex data; second one for the indices
    ...
    GLint stride = cylinder.getInterleavedStride();
...
// Implements the UCreateMesh function for the Pyramid
void createPyramidMesh(GLMesh& mesh)
{
    ...
    GLfloat verts[] = {
        ...
        -0.5f, 0.75f, -0.5f,  1.0f, 0.0f, 0.0f, 1.0f,//back left base corner
        0.5f,  0.75f, -0.5f,  0.0f, 1.0f, 0.0f, 1.0f,//back right base corner
        0.5f,  0.75f,  0.5f,  0.0f, 0.0f, 1.0f, 1.0f,//front right base corner
        -0.5f, 0.75f,  0.5f,  0.5f, 0.5f, 0.0f, 1.0f,//front left base corner
        0.0f,  2.25f,  0.0f,  1.0f, 1.0f, 1.0f, 1.0f,//point     
